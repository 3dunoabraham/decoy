...
export default function Component({
  ...
  useFrame((state: any, delta) => {
    ...
    if (meshRef.current && state && state.get) {
      ...
      let parsedElapsed = parseInt(delta + elapsed)
        if (parsedElapsed > liveId) {
          console.log("asqweqwed", parsedElapsed, delta + elapsed, initUnix, liveUnix)
          meshRef.current.position.x += wallWidth * 2
          s__liveId(liveId+1)
  ...
  return (
    ...
    <Instances
      ...
      xRange={[-boundaries[0], boundaries[0]*100]}
      yRange={[-100, 1]}
    />
